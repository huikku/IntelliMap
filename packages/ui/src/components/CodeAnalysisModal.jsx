...
`;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-navy border-2 border-slate rounded-lg shadow-2xl max-w-3xl w-full mx-4 max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className={`px-6 py-4 border-b ${hasIssues ? 'border-gold bg-gold/20' : 'border-teal bg-teal/20'}`}>
        ...
        {/* Tabs */}
        <div className="flex border-b border-teal/30 bg-slate">
          {['overview', 'issues', 'metrics'].map(tab => (
            <button
              ...
              className={`px-4 py-2 text-sm font-semibold transition ${
                activeTab === tab
                  ? 'bg-navy text-cream border-b-2 border-teal'
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-slate rounded-lg p-4 border border-teal/30">
                  <div className="text-2xl font-bold text-teal">{analysis.totalNodes}</div>
                  <div className="text-xs text-mint uppercase tracking-wide">Files</div>
                </div>
                <div className="bg-slate rounded-lg p-4 border border-teal/30">
                  <div className="text-2xl font-bold text-sage">{analysis.totalEdges}</div>
                  <div className="text-xs text-mint uppercase tracking-wide">Dependencies</div>
                </div>
                <div className="bg-slate rounded-lg p-4 border border-teal/30">
                  <div className="text-2xl font-bold text-peach">{avgDependencies.toFixed(1)}</div>
                  <div className="text-xs text-mint uppercase tracking-wide">Avg Deps/File</div>
                </div>
              </div>

              {/* Quick Stats */}
              <div className="bg-slate border border-teal/30 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-mint">Maximum Depth:</span>
                  <span className="text-cream font-semibold">{maxDepth} levels</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-mint">Largest Component:</span>
                  <span className="text-cream font-semibold">{largestComponents[0]?.size || 0} files</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-mint">Hub Files (high fan-out):</span>
                  <span className="text-cream font-semibold">{hubNodes.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-mint">Leaf Files (high fan-in):</span>
                  <span className="text-cream font-semibold">{leafNodes.length}</span>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="bg-red-950 border border-red-800 rounded-lg p-4">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {godFiles.map((node, idx) => (
                      <div key={idx} className="bg-[#000000] rounded p-2 font-mono text-xs text-[#a0a0a0]">
                ...
                <div className="bg-yellow-950 border border-yellow-800 rounded-lg p-4">
                  ...
                  <div className="space-y-1 max-h-48 overflow-y-auto">
                    {orphanNodes.map((node, idx) => (
                      <div key={idx} className="font-mono text-xs text-[#a0a0a0]">
                        {node.id}
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'metrics' && (
            <div className="space-y-4">
              ...
              <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg p-4">
                ...
                <div className="text-xs text-[#6a6a6a] mb-3">
                  Files that import many other files
                </div>
                {hubNodes.length > 0 ? (
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {hubNodes.map((node, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-[#000000] rounded p-2">
                        <span className="font-mono text-xs text-[#a0a0a0] truncate">{node.id}</span>
